...
function ReviewPage() {
    ...
    return (
        <div className="container max-w-xl mx-auto px-6 mt-10 lg:mt-20">
            <Modal
                isOpen={showSuccess}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* 내용 */}
                <div>
                    <label className="block font-light text-gray-700 mb-2">리뷰 내용</label>
                    <textarea
                        value={content}
                        onChange={(e) => setContent(e.target.value)}
                        className="w-full border border-gray-300 rounded-none px-4 py-3 text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-400 min-h-[140px]"
                ...
                {/* 이미지 업로드 */}
                <div>
                    <label className="block font-light text-gray-700 mb-2">이미지 첨부</label>
                    <input
                        type="file"
                        accept="image/*"
                        multiple
                        onChange={handleImageChange}
                        className="block w-full border border-gray-200 rounded-none text-gray-700 bg-white
                   file:mr-4 file:py-2 file:px-4 file:rounded-none file:border-0
                   file:text-sm file:font-light file:bg-black file:text-white file:appearance-none
                   transition"
                    />
                </div>
                ...
                <button
                    type="submit"
                    disabled={loading}
                    className="w-full py-3 bg-black text-white rounded-none font-light"
                >
                    {loading ? '등록 중...' : '리뷰 등록'}
                </button>
