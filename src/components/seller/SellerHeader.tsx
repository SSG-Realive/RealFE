...
export default function SellerHeader({ toggleSidebar }: SellerHeaderProps) {
    ...
    return (
        <header className="flex items-center justify-between px-8 py-3 bg-[#23272a] border-b border-[#23272a] shadow-sm">
            <div className="flex items-center gap-3">
                {toggleSidebar && (
                    <button
                        onClick={toggleSidebar}
                        className="block lg:hidden text-2xl text-[#fff] hover:text-[#a89f91] transition-colors"
                        style={{ background: 'none', border: 'none' }}
                    >
                        ☰
                    </button>
                )}
                <Link href="/seller/dashboard" className="text-2xl font-extrabold text-[#fff] tracking-tight hover:text-[#a89f91] transition-colors">
                    Realive
                </Link>
            </div>
            <div className="flex items-center gap-6">
                {accessToken ? (
                    <>
                        <span className="font-semibold text-[#fff]">{name}님</span>
                        <button
                            onClick={handleLogout}
                            className="text-[#fff] hover:text-[#a89f91] font-semibold transition-colors"
                    ...
                    <Link href="/seller/login" className="font-semibold text-[#fff] hover:text-[#a89f91] transition-colors">
                        로그인
                    </Link>
                )}
            </div>
